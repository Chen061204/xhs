...
class AnalyzeRequest(BaseModel):
    ...
    category: NonEmptyText | None = Field(default=None, description="可选的热点分类")
    summary: NonEmptyText | None = Field(default=None, description="可选的热点摘要")
    context: NonEmptyText | None = Field(default=None, description="可选的补充上下文")
...
class TrendSource(StrictSchema):
    ...
    url: AnyHttpUrl = Field(description="公开来源 URL")


class TrendingItem(StrictSchema):
    rank: int = Field(ge=1, le=20, description="当前列表内的趋势排序")
    title: NonEmptyText = Field(description="适合小红书创作的趋势选题")
    metrics: NonEmptyText = Field(description="可核验的公开热度描述或公开数据未披露")
    category: NonEmptyText = Field(description="选题所属赛道")
    summary: NonEmptyText = Field(description="选题摘要")
    heat_reason: NonEmptyText = Field(description="趋势选题的推演依据")
    ...
    sources: list[TrendSource] = Field(
        ...
        max_length=3,
        description="腾讯云 TokenHub 联网搜索返回并经后端校验的公开来源",
    )


class TrendingResponse(StrictSchema):
    date: CalendarDate = Field(description="趋势生成日期，Asia/Shanghai")
    items: list[TrendingItem] = Field(
        min_length=1,
        max_length=20,
        description="今日趋势选题列表",
    )
    disclaimer: NonEmptyText = Field(description="趋势结果的能力与时效说明")
